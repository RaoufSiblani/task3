const Section7 = () => { return (
    <div className="text-white">
      <div
        className="absolute left-[8.33333%] top-auto bg-footergreen rounded-xl w-10/12 z-20 "
      >
        <div className="flex flex-col justify-center items-center">
          <h3
            className="font-bold text-xl lg:my-10 my-5 leading-tight text-center lg:text-4xl "
          >Register your CV now!</h3>
          <p className="text-lg lg:text-2xl text-textgray text-center">
            Lorem ipsum dolor sit amet, consectetur adipiscing elit. Sed quis lacus
            non orci euismod vestibulum vitae ut ex.
          </p>
          <button className="border border-white px-4 py-3 rounded-xl font-bold text-lg lg:text-2xl lg:my-10 my-5 hover:bg-white hover:text-black transition-colors">
            <span>Register Now</span>
          </button>
        </div>
      </div>
      <div className="w-full pt-28 ">
  <div className="grid grid-cols-12 bg-maingreen py-32 lg:hidden px-[8.3333%] items-center">
    <div className="col-span-9 mt-10">
      <a href="/">
        <img src="/images/logow.png" alt="Logo" className="h-9" />
      </a>
      <p className="text-sm pt-6">
        Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nunc consequat facilisis nibh vel faucibus.
      </p>
      <h3 className="text-xl font-bold pt-16">Company</h3>
      <ul className="text-xs mt-3">
        <li>Address: 123 Fifth Avenue, New York - 1060, USA.</li>
        <li>Call Us : +(1600) 456 7890</li>
        <li>Email : yourid@example.com</li>
        <li>Mon Sat: 9:00 AM - 19:00 PM</li>
      </ul>
      <h3 className="text-xl font-bold pt-16">Contact Us</h3>
      <div className="inline-flex mt-3">
        <a><img src="/images/google.png" alt="google" className="pr-2 h-7"/> </a>
        <a><img src="/images/facebook.png" alt="facebook" className="pr-2 h-7"/> </a>
        <a><img src="/images/twitter.png" alt="twitter" className="pr-2 h-7"/> </a>
        <a><img src="/images/linkedin.png" alt="linkedin" className="pr-2 h-7"/> </a>
      </div>
    </div>
    <div className="col-span-3 flex items-center pt-28">
      <ul>
        <li className="text-xl font-bold mb-3">
          <h3>Pages</h3>
        </li>
        <li className="text-xs">
          <a href="/">&gt; Home</a>
        </li>
        <li className="text-xs">
          <a href="/">&gt; Booking</a>
        </li>
        <li className="text-xs">
          <a href="/">&gt; Facilities</a>
        </li>
        <li className="text-xs">
          <a href="/">&gt; About Us</a>
        </li>
        <li className="text-xs">
          <a href="/">&gt; Location</a>
        </li>
        <li className="text-xs">
          <a href="/">&gt; Contact</a>
        </li>
      </ul>
    </div>
  </div>
  <div className=" bg-maingreen">
  <div className="lg:grid grid-cols-12  py-32 hidden 2xl:container 2xl:mx-auto">
    <div className=" col-span-4 mt-40 pr-48 pl-16">
    <a href="/">
        <img src="/images/logow.png" alt="Logo" className=" w-3/4" />
      </a>
      <p className="text-lg pt-6">
        Lorem ipsum dolor sit amet, consectetur adipiscing elit. Nunc consequat facilisis nibh vel faucibus.
      </p>
    </div>
    <div className=" col-span-4 mt-32 pr-48">
    <h3 className="text-3xl font-bold">Company</h3>
      <ul className="text-lg mt-3">
        <li>Address: 123 Fifth Avenue, New York - 1060, USA.</li>
        <li>Call Us : +(1600) 456 7890</li>
        <li>Email : yourid@example.com</li>
        <li>Mon Sat: 9:00 AM - 19:00 PM</li>
      </ul>
    </div>
    <div className=" col-span-2 mt-32">
    <ul>
        <li className="text-3xl font-bold mb-3">
          <h3>Pages</h3>
        </li>
        <li className="text-lg">
          <a href="/">&gt; Home</a>
        </li>
        <li className="text-lg">
          <a href="/">&gt; Booking</a>
        </li>
        <li className="text-lg">
          <a href="/">&gt; Facilities</a>
        </li>
        <li className="text-lg">
          <a href="/">&gt; About Us</a>
        </li>
        <li className="text-lg">
          <a href="/">&gt; Location</a>
        </li>
        <li className="text-lg">
          <a href="/">&gt; Contact</a>
        </li>
      </ul>
    </div>
    <div className=" col-span-2 mt-32">
    <h3 className="text-3xl font-bold ">Contact Us</h3>
      <div className="inline-flex mt-3">
        <a><img src="/images/google.png" alt="google" className="pr-2 h-10"/> </a>
        <a><img src="/images/facebook.png" alt="facebook" className="pr-2 h-10"/> </a>
        <a><img src="/images/twitter.png" alt="twitter" className="pr-2 h-10"/> </a>
        <a><img src="/images/linkedin.png" alt="linkedin" className="pr-2 h-10"/> </a>
      </div>
    </div>
  </div>
  </div>
</div>
    </div>
    
    ); }; export default Section7;